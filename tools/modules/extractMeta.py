...
class MetadataExtractor:
    ...
    def get_metadata(pdf_file_path):
        ...
        with open(pdf_file_path) as f:
            ...
            md = pdf_reader.getDocumentInfo()
            # md = {k.lower().replace('/', ''): v for k, v in md.items()}
            # resolve the IndirectObjects from pyPdf
            md = {k.lower().replace('/', ''): v.getObject() if v.__class__.__name__ == 'IndirectObject' else v for k, v in md.items()}
            return md
